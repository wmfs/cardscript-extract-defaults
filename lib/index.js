...
module.exports = async function extractDefaults (cardscript) {
  ...
  function parseElement (element) {
    ...
    switch (element.type) {
      ...
      case 'Collapsible':
        if (element.id) {
          applyDefaultValue(defaultValues, defaultValue === true, apiLookupPath, cardListPath, element.id)
        }
      ...
      case 'Input.Signature':
        ...
        if (element.allowNameInput) {
          applyDefaultValue(defaultValues, defaultValue, apiLookupPath, cardListPath, `${element.id}Name`)
        }
        break
      case 'AdaptiveCard': {
      ...
      case 'Input.Chat':
        if (element.id) {
          applyDefaultValue(defaultValues, null, apiLookupPath, cardListPath, `${element.id}ChatInput`)
          applyDefaultValue(defaultValues, [], apiLookupPath, cardListPath, `${element.id}ChatMessages`)
          applyDefaultValue(defaultValues, {}, apiLookupPath, cardListPath, 'chatControls')
        }
        break
    }
  }
